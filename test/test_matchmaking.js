...
const checkTradeProposalAccepted = async (
  t,
  agent,
  agentName,
  itemsNumber,
  ...
  const totalTradeProposals = await getTradeProposals(agent);
  t.equal(
    totalTradeProposals.Ok.length,
    itemsNumber - 1,
    `${agentName} can see ${itemsNumber - 1} proposal(s)`,
  );
  t.equal(
    totalTradeProposals.Ok[0].address,
    discardedAddress,
    `${agentName} sees the correct discarded option`,
  );
};

module.exports = scenario => {
  scenario('Alice can create trade proposal', async (s, t, { alice }) => {
    await createTradeProposal(alice, 'Socks', 'Old and smelly');
    t.equal(
      lastResult().Ok.length,
      addressLength,
    ...
    async (s, t, { alice, bob }) => {
      ...
      for (const [item, desc] of Object.entries(items)) {
        ...
        t.equal(
          lastResult().Ok.length,
          addressLength,
          `Alice created ${item} proposal successfully`,
        );
      }
  ...
  scenario(
    'Alice creates 2 trade proposals and Bob accepts one trade proposal',
    async (s, t, { alice, bob }) => {
      ...
      t.equal(
        lastResult().Ok.length,
        itemsNumber,
        `Bob can see ${itemsNumber} proposals`,
      );
      console.log(JSON.stringify(lastResult(), null, tabLength));
      ...
      await checkTradeProposalAccepted(
        t,
        alice,
        'Alice',
        itemsNumber,
